...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        f"👉 [Click Here to Join](https://t.me/{INFLICT_DDOS})\n"
        "After joining the channel, click 'I Have Joined' to proceed."
    )
    
    keyboard = [
        [InlineKeyboardButton("📢 Join Channel", url=f"https://t.me/{INFLICT_DDOS}")],
...
async def genkey(update: Update, context: CallbackContext):
    if update.callback_query:  # Check if called from a button click
        ...
        if user_id in ADMIN_IDS:
            ...
            await context.bot.send_message(user_id, "ONLY OWNER CAN USE💀OWNER @INFLICT_BGMI")
        ...
        if user_id in ADMIN_IDS:
            ...
            response = "ONLY OWNER CAN USE💀OWNER @INFLICT_BGMI"
